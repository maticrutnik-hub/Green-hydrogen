...
PV_area=132.6;           % [m2] (power plant area)
BattCapacity=1*6;      % [kWh], Battery capacity
...
HsysOutPowerMax=6.2;     % [kW], max discsharge power (fuel cell power)
BattInpPowerMax=1*8;    % [kW], max battery charge power
BattOutPowerMax=1*8;    % [kW], max battery discharge power
...
CAPEX_r_Electrolyser = 400;   %EUR/kW
CAPEX_r_FuelCell = 400;       %EUR/kW
CAPEX_r_HydrogenTank = 125;   %EUR/kg
CAPEX_r_Battery = (400 / (0.8*0.75))/2;  %EUR/kWh %0.8 DoD, 0.75 Degradacija, *2 - change in 10 years, /2 subsidy
...
OPEX_HydrogenTank = 0;
OPEX_Battery = (400 / (0.8*0.75))/2 *BattCapacity;
