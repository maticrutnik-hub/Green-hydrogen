...
PV_area=147.7;           % [m2] (power plant area)
% PV_area=135;           % [m2] (power plant area)
BattCapacity=0*16;      % [kWh], Battery capacity
HsysCapacity=1*8000;   % [kWh], Hydrogen tank capacity
HsysInpPowerMax=13;     % [kW], max charge power (electrolyser power)
HsysOutPowerMax=13;     % [kW], max discsharge power (fuel cell power)
...
HsysSoC(1)=3730;                           % [kWh], Starting value of H2 tank
...
for i=2:n
    ...
    ControlLogic(Ts, ExcessPower(i),...
    BattSoC(i-1), BattCapacity, BattFullFact, BattEmptyFact, BattInpPowerMax, BattOutPowerMax,...
    HsysSoC(i-1), HsysCapacity, HsysFullFact, HsysEmptyFact, HsysInpPowerMax, HsysOutPowerMax);
...
HpPowerM=zeros(365,48);    % [kW]
OtherPowerM=zeros(365,48); % [kW]
...
for di=1:365
        ...
        ExcessPowerM(di,ti) = ExcessPower((di-1)*48+ti);
         ...
         % Heat pump electric power in day (heat pump data, kW)
        HpPowerM(di,ti) = P_HeatPump_el_dem((di-1)*48+ti);
        % Other consumption electric power in day (kW)
        OtherPowerM(di,ti) = P_Other((di-1)*48+ti);
...
%% 3D PLOT: Consumed heat pump electric power as a function of hour-in-day and day-in-year
%----------------------------------------------------------------------
% dMat = (repmat(Days', 1, 48))';
% hMat = (repmat(Hours', 1, 365));
days = 1:1:365;
hours = 0:0.5:23.5;
[Days, Hours] = meshgrid(days, hours);

figure (29);
surf(Days, Hours, HpPowerM');
caxis([-3, 5])
%clim([-3, 5]);
xlabel('Day 1...365');
ylabel('Hour 0...23,5');
zlabel('Consumed HP electric power [kW]');
xlim([1 365]);
ylim([0 24]);
zlim([0 5]);
%title('Consumer heat pump electric power as a function of hour and day');

%% 3D PLOT: Consumed other electric power as a function of hour-in-day and day-in-year
%----------------------------------------------------------------------

figure (30);
surf(Days, Hours, OtherPowerM');
caxis([-3, 5])
xlabel('Day 1...365');
ylabel('Hour 0...23,5');
zlabel('Consumed Other electric power [kW]');
xlim([1 365]);
ylim([0 24]);
zlim([0 6]);
%title('Consumer Other electric power as a function of hour and day');

figure (31);
surf(Days, Hours, 1000*SunPowerM');
caxis([0, 800])
xlabel('Day 1...365');
ylabel('Hour 0...23,5');
zlabel('Solar radiation [W/m2]');
xlim([1 365]);
ylim([0 24]);
zlim([0 1500]);
zticks([0 500 1000 1500])
%title('Sun radiatio as a function of hour and day');
